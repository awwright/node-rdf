...
var prof = new (require('./DataContext.js').Profile);
require('./DefaultDataContext.js').loadDefaultPrefixMap(prof);
				...
				function graphify(s1,p1,o1) {
					...
					if(o1.id) {
						...
						if(!o1.list) {
							...
							if(o1.length == 0) {
								graph.add( new RDFTriple(s1.resolve(), prop(p1,map).resolve(), prof.resolve("rdf:nil") ) );
							} else {
								var b = {}.ref();
								graph.add( new RDFTriple(s1.resolve(), prop(p1,map).resolve(), b.id.resolve() ) );
								o1.forEach( function(i,x) {
									graphify(b.id, prof.resolve('rdf:first'), i );
									var n = {}.ref();
									graph.add( new RDFTriple(b.id.resolve(), 'rdf:rest'.resolve(), (x == o1.length-1) ? prof.resolve('rdf:nil') : n.id.resolve() ) );
									b = n;
								});
							}
						}
...
var StringProperties = {
	tl: _( function(t) {
		var v = new RDFNode.Literal(this.toString());
		v.datatype = t;
		return v;
	}),
	l: _( function(l) {
	...
	resolve: _( function() {
		...
		return prof.resolve(this)||this.toString();
	}),
	...
	n3: _( function() {
		switch(this.nodeType()) {
			...
			case 'TypedLiteral':
				if(this.type.resolve() == prof.resolve("rdf:PlainLiteral")) return '"' + encodeString(this) + '"'; 
...
var BooleanProperties = {
	type: _( prof.resolve("xsd:boolean") ),
...
// Date
var DateProperties = {
	type: _( prof.resolve("xsd:dateTime") ),
	value: _getter(function(){return this;}),
	nodeType: _( function() { return "TypedLiteral"} ),
	n3: _( function() {
...
var NumberProperties = {
	type: {
		...
		get: function() {
			if(this == Number.POSITIVE_INFINITY) return prof.resolve('xsd:double');
			if(this == Number.NEGATIVE_INFINITY) return prof.resolve('xsd:double');
			if(this == Number.NaN) return prof.resolve('xsd:double');
			var n = this.toString();
			if(INTEGER.test(n)) return prof.resolve('xsd:integer');
			if(DECIMAL.test(n)) return prof.resolve('xsd:decimal');
			if(DOUBLE.test(n)) return prof.resolve('xsd:double');
		}
	},
	value: 	 _getter(function(){return this;}),
	nodeType: _( function() { return "TypedLiteral" } ),
	n3: _( function() {
		if(this == Number.POSITIVE_INFINITY) return '"INF"^^<' + prof.resolve('xsd:double') + '>';
		if(this == Number.NEGATIVE_INFINITY) return '"-INF"^^<' + prof.resolve('xsd:double') + '>';
	...
	toNT: _( function() {
		if(this == Number.POSITIVE_INFINITY) return '"INF"^^<' + prof.resolve('xsd:double') + '>';
		if(this == Number.NEGATIVE_INFINITY) return '"-INF"^^<' + prof.resolve('xsd:double') + '>';
		if(this == Number.NaN) return '"NaN"^^<' + prof.resolve('xsd:double') + '>';
