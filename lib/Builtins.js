...
function shrinkprop(ref, profile) {
	if(ref instanceof BlankNode) return ref.toNT();
	if(ref instanceof NamedNode) ref = ref.toString();
	var short = profile.shrink(ref.toString());
	return (short==ref) ? env.createNamedNode(ref).toNT() : short ;
};
...
function createTypedLiteralFrom(value){
	...
	if(value instanceof Date) return new Literal(value.toString(), exports.DateProperties.datatype.value);
...
function n3(value){
	if(typeof value.n3=='function') return value.n3();
	if(typeof value=='string') return exports.StringProperties.n3.value.call(value);
	if(typeof value=='number') return exports.NumberProperties.n3.value.call(value);
	if(typeof value=='boolean') return exports.BooleanProperties.n3.value.call(value);
	if(value instanceof Date) return exports.DateProperties.n3.value.call(value);
	throw new TypeError('Cannot create n3 from '+Object.toString.call(value));
}
...
function graphifyObject(aliasmap){
	...
	function graphify(s1,p1,o1) {
		if(typeof s1=='string') var s1n = createNamedNodeFrom(s1);
		else if(s1 instanceof NamedNode) var s1n = s1;
		else throw new Error('Expected string/NamedNode subject');
		if(p1[0]=='@' || p1[0]=='$') return;
		if(typeof(o1)=='function' || typeof(o1)=='undefined') return;
		if(typeof o1=='object'){
			if(o1 instanceof RDFNode){
				var literal = o1;
			}else if(o1 instanceof Date){
				var literal = exports.DateProperties.toRDFNode.value.call(o1);
			}else if(!o1.id){
				var id = o1.id || o1['$id'] || o1['@id'];
				if(!o1.nodeType && !id && typeof(o1)!="string") o1 = ref.call(o1); // If the Object doesn't have a bnode, give it one
				id = id || o1.id;
			}
		}
		if(Array.isArray(o1) || o1['$list'] || o1['@list']) {
			...
			if(!o1.list) {
				...
				if(o1.length == 0) {
					graph.add( env.createTriple(s1n, expandprop(p1,profile), rdfnil ) );
				} else {
					var bnode = env.createBlankNode();
					graph.add( env.createTriple(s1n, expandprop(p1,profile), bnode ) );
					o1.forEach( function(i,x) {
						graphify(bnode, rdffirst, i );
						var n = env.createBlankNode();
						graph.add( env.createTriple(bnode, rdfrest, (x==o1.length-1) ? rdfnil : n ) );
						bnode = n;
					});
				}
			}
		} else if(literal) {
			graph.add( env.createTriple(s1n, expandprop(p1,profile), literal ) );
		} else if(id) {
			// o1 is an Object, add triple and child triples
			graph.add( env.createTriple(s1n, expandprop(p1,profile), createNamedNodeFrom(id) ) );
			graph.addAll( graphifyObject.call(o1, profile) );
		} else if(typeof o1=='string') {
			// o1 is a string (convert to NamedNode) or RDFNode (NamedNode, BlankNode, or Literal)
			graph.add( env.createTriple(s1n, expandprop(p1,profile), expandnode(o1,profile) ) );
		} else if(typeof o1=='number') {
			graph.add( env.createTriple(s1n, expandprop(p1,profile), exports.NumberProperties.toRDFNode.value.call(o1) ) );
		} else if(typeof o1=='boolean') {
			graph.add( env.createTriple(s1n, expandprop(p1,profile), exports.BooleanProperties.toRDFNode.value.call(o1) ) );
		} else {
...
function ref(id) {
	...
	Object.defineProperties(copy, {
		...
		n3: _( function toN3(aliasmap, padding) {
			...
			Object.keys(this).forEach(function toN3_property(p) {
				...
				if(typeof val=='string'){
					...
					outs.push( predicateString + ' ' + objectName );
					return;
				}
				if(val && val.nodeType && typeof val.nodeType == 'IRI'){
					outs.push( predicateString + ' ' + shrinknode(val.n3(),profile) );
					return;
			...
			if(id){
				if(outs.length > 1) return shrinknode(expandnode(this.id,profile),profile) + padding + outs.join(";"+padding) + ' .';
				if(outs.length == 1) return shrinknode(expandnode(this.id,profile),profile) + ' ' + outs.join(";"+padding) + ' .';
				else return '';
			}
		...
		getProfile: _( function(aliasmap) {
			var o = this;
			// Determine the prefix/term profile this object is using
...
exports.BooleanProperties = {
	...
	toRDFNode: _( function() { return env.createTypedLiteral(this.valueOf().toString(), xsdns("boolean")); } ),
...
exports.DateProperties = {
	...
	n3: _( function() {
		...
		+ pad(this.getUTCHours())+':' + pad(this.getUTCMinutes())+':' + pad(this.getUTCSeconds())+'Z"^^<' + xsdns("dateTime") + '>';
	}),
	toNT: _( function() { return this.n3() } ),
	toRDFNode: _( function() {
		return env.createTypedLiteral(this.getUTCFullYear()+'-' + pad(this.getUTCMonth()+1)+'-' + pad(this.getUTCDate())+'T'
		+ pad(this.getUTCHours())+':' + pad(this.getUTCMinutes())+':' + pad(this.getUTCSeconds())+'Z', xsdns("dateTime"));
	}),
	toCanonical: _( function() { return this.n3() } )
}
...
exports.NumberProperties = {
	...
	toNT: _( function() {
		...
		if(this == Number.NEGATIVE_INFINITY) return '"-INF"^^<' + xsdns('double') + '>';
		if(isNaN(this)) return '"NaN"^^<' + xsdns('double') + '>';
		return '"' + this.toString() + '"' + "^^<" + exports.NumberProperties.datatype.get.call(this) + '>';
	}),
	toRDFNode: _( function() {
		...
		return env.createTypedLiteral(this.toString(), exports.NumberProperties.datatype.get.call(this));
	}),
