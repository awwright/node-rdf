...
function graphifyObject(aliasmap){
	...
	var v;
	if(v=(o['$context']||o['@context'])){
		for(var prefix in v) profile.setPrefix(prefix, v[prefix]);
	}
	function graphify(s1,p1,o1) {
		...
		if(Array.isArray(o1) || o1['$list'] || o1['@list']) {
			...
			if(!o1.list) {
				...
				if(o1.length == 0) {
					graph.add( new Triple(res(s1), res(prop(p1,profile)), env.resolve("rdf:nil") ) );
				} else {
					...
					o1.forEach( function(i,x) {
						...
						var n = ref.call({});
						graph.add( new Triple(res(b.id), 'rdf:rest'.resolve(), (x == o1.length-1) ? env.resolve('rdf:nil') : res(n.id) ) );
						b = n;
					});
				}
			}
...
function ref(id) {
	...
	Object.defineProperties(this, {
		id: _( id ? id.resolve() : (new BlankNode).toString() ),
		n3: _( function(aliasmap, padding) {
			...
			Object.keys(this).forEach(function(p) {
				...
				outs.push( padding + prop(p, profile) + ' ' + o[p].n3(profile, padding+'\t') );
			});
			outs = outs.join(";");
			return id ? this.id.n3(undefined)+''+outs+' .' : '['+outs+' ]';
		}),
		toNT: _( function(a) {
