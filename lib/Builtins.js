				...
				function graphify(s1,p1,o1) {
					...
					if(o1.id) {
						...
						if(!o1.list) {
							...
							if(o1.length == 0) {
								graph.add( new RDFTriple(s1.resolve(), prop(p1,map).resolve(), env.resolve("rdf:nil") ) );
							} else {
								var b = {}.ref();
								...
								o1.forEach( function(i,x) {
									...
									var n = {}.ref();
									graph.add( new RDFTriple(b.id.resolve(), 'rdf:rest'.resolve(), (x == o1.length-1) ? env.resolve('rdf:nil') : n.id.resolve() ) );
									b = n;
								});
							}
						}
