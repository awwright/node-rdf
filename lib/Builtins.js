	...
	function graphify(s1,p1,o1) {
		...
		if(Array.isArray(o1) || o1['$list'] || o1['@list']) {
			...
			if(!o1.list) {
				...
				if(o1.length == 0) {
					graph.add( env.createTriple(s1n, env.createNamedNode(prop(p1,profile)), rdfnil ) );
				} else {
					var bnode = env.createBlankNode();
					graph.add( env.createTriple(s1n, env.createNamedNode(prop(p1,profile)), bnode ) );
			...
			graph.add( env.createTriple(s1n, env.createNamedNode(prop(p1,profile)), literal ) );
		} else if(id) {
			// o1 is an Object, add triple and child triples
			graph.add( env.createTriple(s1n, env.createNamedNode(prop(p1,profile)), createNamedNodeFrom(id) ) );
			...
			graph.add( env.createTriple(s1n, env.createNamedNode(prop(p1,profile)), createNamedNodeFrom(prop(o1,profile)) ) );
		} else if(typeof o1=='number') {
			graph.add( env.createTriple(s1n, env.createNamedNode(prop(p1,profile)), exports.NumberProperties.toRDFNode.value.call(o1) ) );
		} else if(typeof o1=='boolean') {
			graph.add( env.createTriple(s1n, env.createNamedNode(prop(p1,profile)), exports.BooleanProperties.toRDFNode.value.call(o1) ) );
		} else if(o1 instanceof RDFNode) {
			graph.add( env.createTriple(s1n, env.createNamedNode(prop(p1,profile)), o1 ) );
		} else if(o1 instanceof Date) {
			graph.add( env.createTriple(s1n, env.createNamedNode(prop(p1,profile)), exports.DateProperties.toRDFNode.value.call(o1) ) );
		} else {
		...
		n3: _( function toN3(aliasmap, padding) {
			...
			Object.keys(this).forEach(function toN3_property(p) {
				...
				if(typeof val=='string'){
					...
					outs.push( predicateString + ' ' + shrink(val, profile) );
					return;
