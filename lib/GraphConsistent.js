...
// Verify that every statement in `tst` is true according to `ref`
// Specifically: tst may be a subset of ref,
// and bnodes in tst may map to either bnodes or namednodes in ref.

module.exports = GraphConsistent;
function GraphConsistent(ref, tst, depth){
	...
	while(stack.length){
		...
		var matches = ref.match(stmtsubject, stmtpredicate, stmtobject).filter(function(m){
			...
			if(stmtobject===null && (m.object.termType!=='BlankNode' && m.object.termType!=='NamedNode')) return false;
			return true;
		});
