...
function RDFNodeEquals(other) {
	if(api.RDFNode.is(other)){
		...
		switch(nodeType(this)) {
			...
			case "TypedLiteral":
				return this.datatype.toString()==other.datatype.toString() && this.nominalValue==other.nominalValue;
		}
...
function RDFNodeCompare(other) {
	...
	var typeThis=nodeType(this), typeOther=nodeType(other);
	...
	if(typeof this.nominalValue=='string' && typeof other.nominalValue=='string'){
		...
		if(this.nominalValue > other.nominalValue) return 1;
		if(this.nominalValue == other.nominalValue) return 0;
	}
	// values are the same, compare by datatype
	if(typeof this.datatypeIRI=='string' && typeof other.datatypeIRI=='string'){
		if(this.datatypeIRI < other.datatypeIRI) return -1;
		if(this.datatypeIRI > other.datatypeIRI) return 1;
		if(this.datatypeIRI == other.datatypeIRI) return 0;
	}
...
function Literal(value, type) {
	...
	if(type!==null && type!==undefined && typeof type!='string' && !(type instanceof api.NamedNode)) throw new TypeError('Expected optional argument[1] `type` to be a string');
	...
	if(typeof type=='string'){
		if(type.match(/^[a-zA-Z]+(-[a-zA-Z0-9]+)*$/)){
			...
			// xsd:string is the same as no datatype in RDF1.1 semantics
			if(type.toString()!=='http://www.w3.org/2001/XMLSchema#string'){
...
api.Literal.prototype.n3 = function(profile){
	if(profile && this.datatype){
...
api.TriplePattern.prototype.toString = function() { return this.subject.n3() + " " + this.predicate.n3() + " " + this.object.n3() + " ." }
api.TriplePattern.prototype.equals = function(t) { return RDFNodeEquals.call(this.subject,t.subject) && RDFNodeEquals.call(this.predicate,t.predicate) && RDFNodeEquals.call(this.object,t.object) }
...
api.RDFNode.prototype.toNT = function() { throw new Error('Cannot serialize variable to N-Triples'); }
