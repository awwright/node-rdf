...
function RDFNodeCompare(other) {
	...
	if(typeof this.type=='string' && typeof other.type=='string'){
...
api.Triple.prototype.toTurtle = function toTurtle() {
...
api.Literal.prototype.toTurtle = function toTurtle(profile){
	if(this.datatype.toString()==xsdinteger && this.value.match(INTEGER)){
		return this.value;
	}
	if(this.datatype.toString()==xsddecimal && this.value.match(DECIMAL)){
		return this.value;
	}
	if(this.datatype.toString()==xsddouble && this.value.match(DOUBLE)){
		return this.value;
	}
