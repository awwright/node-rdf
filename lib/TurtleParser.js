...
var rdfns = function rdfns(v){return 'http://www.w3.org/1999/02/22-rdf-syntax-ns#'+v;};
...
Turtle.prototype.parse = function parse(doc, callback, base, filter, graph) {
	this.graph = graph==null ? this.environment.createGraph() : graph;
...
Turtle.prototype.consumeCollection = function(s, subject) {
	...
	while(cont) {
		...
		this.add(new Triple(listject.o, rdfns("first"), o.o));
		s = this.skipWS(s);
		cont = s.charAt(0) != ")";
		if(cont) {
			this.add(new Triple(listject.o, rdfns("rest"), listject.o = this.environment.createBlankNode()));
		} else {
			this.add(new Triple(listject.o, rdfns("rest"), rdfns("nil")));
		}
	}
	return this.skipWS(s.slice(1));
};
	...
	while(cont) {
		...
		if(predicate == "a") {
			...
			switch(predicate.charAt(0)) {
				case "<": property = this.base.resolveReference(parsers.decodeString(predicate.substring(1, predicate.indexOf(">")))).toString(); break;
				case "]": return s;
				case ".": return s;
...
Turtle.prototype.consumeURI = function(s, t) {
	this.expect(s, "<");
	var p = 0;
	t.o = this.environment.createNamedNode(this.base.resolveReference(parsers.decodeString(s.substring(1, p=s.indexOf(">")))).toString());
	return s.slice(++p);
};
