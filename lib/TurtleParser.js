...
parsers.Turtle.prototype = {
	...
	consumeCollection: function(s, subject) {
		...
		s = this.skipWS(s.slice(1));
		var cont = s.charAt(0) != ")";
		if(!cont) { subject.o = this.environment.resolve("rdf:nil") }
		while(cont) {
			var o = this.t();
			switch(s.charAt(0)) {
			...
			this.add(new Triple(listject.o, this.environment.resolve("rdf:first"), o.o));
			s = this.skipWS(s);
			cont = s.charAt(0) != ")";
			if(cont) {
				this.add(new Triple(listject.o, this.environment.resolve("rdf:rest"), listject.o = new BlankNode))
			} else {
				this.add(new Triple(listject.o, this.environment.resolve("rdf:rest"), this.environment.resolve("rdf:nil")))
			}
		}
		return this.skipWS(s.slice(1))
	},
	consumeDirective: function(s) {
		...
		if(s.substring(1, 7) == "prefix") {
			...
			this.expect(s, "<");
			this.environment.setPrefix(prefix, parsers.decodeString(s.substring(1, p = s.indexOf(">"))));
			s = this.skipWS(s.slice(++p))
		} else if(s.substring(1, 5) == "base") {
			s = this.skipWS(s.slice(5));
			this.expect(s, "<");
			this.environment.base = this.environment.createNamedNode(parsers.decodeString(s.substring(1, p = s.indexOf(">"))));
			s = this.skipWS(s.slice(++p))
		} else {
	...
	consumePredicateObjectList: function(s, subject) {
		...
		while(cont) {
			...
			var property = null;
			if(predicate == "a") {
				property = this.environment.resolve("rdf:type")
			} else {
				switch(predicate.charAt(0)) {
					case "<": property = this.environment.createNamedNode(parsers.decodeString(predicate.substring(1, predicate.indexOf(">")))); break;
	...
	consumeURI: function(s, t) {
		this.expect(s, "<");
		var p = 0;
		t.o = this.environment.createNamedNode(parsers.decodeString(s.substring(1, p=s.indexOf(">"))));
		return s.slice(++p)
	},
	expect: function(s, t) {
