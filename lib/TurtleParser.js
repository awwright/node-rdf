...
parsers.Turtle.prototype = {
	...
	consumeCollection: function(s, subject) {
		...
		while(cont) {
			...
			this.add(new RDFTriple(listject.o, this.context.resolve("rdf:first"), o.o));
			s = this.skipWS(s);
			cont = s.charAt(0) != ")";
			if(cont) {
				this.add(new RDFTriple(listject.o, this.context.resolve("rdf:rest"), listject.o = new BlankNode))
			} else {
				this.add(new RDFTriple(listject.o, this.context.resolve("rdf:rest"), this.context.resolve("rdf:nil")))
			}
		}
		return this.skipWS(s.slice(1))
	},
	...
	consumeLiteral: function(s, o) {
		...
		switch(s.charAt(0)) {
			case "@":
				var token = s.match(parsers.Turtle.simpleObjectToken).shift();
				o.o = new PlainLiteral(value, token.slice(1));
				s = s.slice(token.length);
				break;
			case "^":
				var token = s.match(parsers.Turtle.simpleObjectToken).shift().slice(2);
				if(token.charAt(0) == "<") {
					o.o = new TypedLiteral(value, token.substring(1, token.length - 1))
				} else {
					o.o = new TypedLiteral(value, token)
				}
				s = s.slice(token.length + 2);
				break;
			default:
				o.o = new PlainLiteral(value);
				break
		}
		return s
	},
	consumeObjectList: function(s, subject, property) {
		...
		while(cont) {
			...
			this.add(new RDFTriple(subject.o, property, o.o));
			s = this.skipWS(s);
			cont = s.charAt(0) == ",";
