...
Turtle.prototype.consumeCollection = function(s, subject) {
	...
	while(cont) {
		...
		cont = s.charAt(0) != ")";
		if(cont) {
			this.add(env.createTriple(listject.o, env.createNamedNode(rdfns("rest")), listject.o = this.environment.createBlankNode()));
		} else {
		...
		switch(s.charAt(0)) {
			...
			default:
				...
				if(token == "false" || token == "true") {
					...
					o.o = this.environment.createNamedNode(this.environment.resolve(token));
