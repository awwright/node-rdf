	...
	switch(s.charAt(0)) {
		...
		case "^":
			...
			if(s.charAt(0) == "<"){
				...
				if(!iri) throw new Error('Prefix not defined for '+prefixedName);
		...
		switch(s.charAt(0)) {
			...
			default:
				...
				if(token==="true" || token==="false"){
					...
					o.o = env.createNamedNode(this.environment.resolve(parsers.decodePrefixedName(prefixedName[0])));
...
Turtle.prototype.consumePredicateObjectList = function(s, subject) {
	...
	while(cont) {
		...
		if(predicate){
			...
			property = env.createNamedNode(this.environment.resolve(parsers.decodePrefixedName(predicate)));
...
Turtle.prototype.consumePrefixedName = function(s, t) {
	...
	t.o = env.createNamedNode(this.environment.resolve(parsers.decodePrefixedName(name)));
