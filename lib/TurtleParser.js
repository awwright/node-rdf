...
parsers.Turtle.prototype = {
		...
		while(cont) {
			...
			if(predicate == "a") {
				...
				switch(predicate.charAt(0)) {
					case "<": property = this.base.resolveReference(parsers.decodeString(predicate.substring(1, predicate.indexOf(">")))); break;
	...
	consumeURI: function(s, t) {
		this.expect(s, "<");
		var p = 0;
		t.o = this.base.resolveReference(parsers.decodeString(s.substring(1, p=s.indexOf(">"))));
		return s.slice(++p)
	},
	expect: function(s, t) {
